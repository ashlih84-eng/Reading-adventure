...
export const playerRepository:PlayerRepository={async list(){return(await(await db).getAll('players')).map(player=>({...player,equippedItems:player.equippedItems??[]}))},async save(player){await(await db).put('players',player)},async remove(id){await(await db).delete('players',id)}};
